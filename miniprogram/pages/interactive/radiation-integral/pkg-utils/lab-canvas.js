...
// 同一页面实例 + 同一 selector 只做一次 SelectorQuery/尺寸设定（缓存 ctx），
// 后续调用直接同步回调——动画/每键重绘不再反复查询节点、重建 backing store。
// 页面回调里应自行 lc.clear() 清屏（原实现里重设 canvas.width 的隐式清屏不再发生）。
// 节点未就绪（如 onLoad 早于首帧）时最多重试 5 次×50ms，页面无需 onLoad+onReady 双调兜底。
function mount(page, selector, cb) {
  const cache = page.__lcMount || (page.__lcMount = {})
  const hit = cache[selector]
  if (hit) { cb(hit.ctx, hit.w, hit.h, hit.canvas); return }
    ...
    wx.createSelectorQuery().in(page).select(selector)
      .fields({ node: true, size: true }).exec((res) => {
        ...
        if (!info || !info.node || !(info.width > 0) || !(info.height > 0)) {
          if (n < 5) setTimeout(() => attempt(n + 1), 50)
          return
        }
        const canvas = info.node
        const ctx = canvas.getContext('2d')
        // 基础库 < 2.20.1 无 getWindowInfo，回退 getSystemInfoSync
        let dpr = 2
        try { dpr = wx.getWindowInfo().pixelRatio || 2 } catch (e) { dpr = (wx.getSystemInfoSync().pixelRatio) || 2 }
        const w = info.width, h = info.height
        canvas.width = Math.max(1, Math.floor(w * dpr))
        canvas.height = Math.max(1, Math.floor(h * dpr))
        ctx.setTransform(dpr, 0, 0, dpr, 0, 0)
        cache[selector] = { ctx, w, h, canvas }
        cb(ctx, w, h, canvas)
      })
  }
  attempt(0)
...
// 强制重新挂载（如布局变化后需要按新尺寸初始化）
function remount(page, selector, cb) {
  const cache = page.__lcMount
  if (cache) delete cache[selector]
  mount(page, selector, cb)
}
...
module.exports = {
  mount, remount, clear, niceTicks, plot,
