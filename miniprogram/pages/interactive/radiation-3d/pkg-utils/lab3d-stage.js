...
// 新版（暖纸舞台）用法：
//   通过各 3D 分包的适配层调用，传入当前分包的 createScopedThreejs。
  ...
  const p = new Promise((resolve, reject) => {
    ...
    const attempt = () => {
      page.createSelectorQuery().select(selector).fields({ node: true, size: true }).exec((res) => {
        ...
        try {
        const canvas = r.node;
        const createScopedThreejs = opts.createScopedThreejs;
        const THREE = canvas.__labScopedThree || (canvas.__labScopedThree = createScopedThreejs(canvas));
...
function init2D(page, selector, cb) {
  return require('./lab-canvas').mount(page, selector, cb);
}
...
function clearGroup(g) {
  ...
  g.traverse(o=>{if(o===g)return;if(o.geometry)geometries.add(o.geometry);
...
function scheduleIdle(page) {
  ...
  page._idleT = setTimeout(() => {
    if (page.controls && page.animId) page.controls.autoRotate = !!(page.data.autoRotate || (page.state && page.state.rotate));
  }, 3000);
}
...
function ready(page) {
  if(page.onLabView){require('./lab3d-visual').fitView(page,'perspective');page._home=saveHome(page.controls);}
